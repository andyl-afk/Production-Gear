...
function GearCard({item,onClick,onLongPress}){
  ...
  return<div onClick={()=>onClick(item)} onMouseDown={start} onMouseUp={end} onMouseLeave={end} onTouchStart={start} onTouchEnd={end}
    style={{background:"#fff",borderRadius:18,padding:"18px 16px",boxShadow:"0 2px 12px rgba(0,0,0,0.07)",cursor:"pointer",border:item.status!=="available"?"1.5px solid #FFE0E0":"1.5px solid transparent",transform:pressed?"scale(0.96)":"scale(1)",transition:"transform 0.1s",display:"flex",flexDirection:"column",gap:10}}>
    ...
    <div style={{flex:1}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,marginTop:4,flexWrap:"wrap"}}>
        {item.qty>1&&<span style={{fontSize:12,fontWeight:600,color:"#007AFF",background:"#EAF2FF",borderRadius:6,padding:"2px 7px"}}>×{item.qty}</span>}
        <span style={{fontSize:12,color:"#8E8E93"}}>{item.group}</span>
      </div>
    </div>
    {item.status==="checkedout"&&item.who&&<div style={{borderTop:"1px solid #F2F2F7",paddingTop:10}}><div style={{fontSize:12,color:"#8E8E93"}}>With</div><div style={{fontSize:13,fontWeight:600,color:"#1C1C1E"}}>{item.who}</div>{item.ret&&<div style={{fontSize:12,color:"#FF9500",marginTop:2}}>↩ Back by {fmt(item.ret)}</div>}</div>}
    {item.status==="damaged"&&item.notes&&<div style={{fontSize:12,color:"#FF3B30",background:"#FFF5F5",borderRadius:8,padding:"6px 10px"}}>⚠️ {item.notes}</div>}
    <div style={{fontSize:11,color:"#C7C7CC",textAlign:"right",marginTop:-4}}>Hold to edit</div>
  </div>
}

function EditSheet({item,isNew,onSave,onDelete,onClose}){
  const[f,setF]=useState(isNew?{name:"",qty:1,cat:"Camera",group:"Shared Pool",notes:""}:{name:item.name,qty:item.qty,cat:item.cat,group:item.group,notes:item.notes||""});
  ...
  return<Overlay onClose={onClose}>
    ...
    <Section mt={16}><SLabel>Name</SLabel><div style={{padding:"0 16px 14px"}}><SInput value={f.name} onChange={e=>s("name",e.target.value)} placeholder="e.g. Sony FX6"/></div></Section>
    <Section>
    ...
    <Btn label={isNew?"Add to Inventory":"Save Changes"} onClick={()=>onSave(f)} disabled={!f.name.trim()} mt={16}/>
...
export default function GearRoom(){
  ...
  const handleSaveItem=form=>{
    if(modal==="add"){setGear(g=>[...g,{id:nextId(g),...form,status:"available",who:null,ret:null}]);showToast(`✓ ${form.name} added`);}
    else{setGear(g=>g.map(x=>x.id===sel.id?{...x,...form}:x));showToast("✓ Changes saved");}
    close();
  };
